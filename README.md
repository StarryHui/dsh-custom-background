...
自定义 DeepSeek Harness（DSH）WebUI 背景插件。在设置侧边栏「agent 预设」下方提供「背景」页面，全部调整实时生效，刷新 / 重启后保持。

## 功能

- **图库背景**：指定本地文件夹，自动扫描其中的图片（png/jpg/webp/gif/bmp/avif/svg），支持缩略图点选与「随机一张」；图库路径支持「浏览…」调起系统文件夹选择器。
- **自由拉伸裁切框**：预览图上拖拽 4 角 / 4 边非等比缩放、框内平移，实时预览背景。
- **效果调节**：水平/垂直缩放（100–800，0.01 分度）、位置、背景不透明度、面板透明度、高斯模糊（0–80）。
- **显示模式**：cover（铺满裁剪）/ contain（完整显示）/ fill（拉伸填充）。
- **多图交叉淡化轮播**：按图库顺序定时切换，切换时双图层交叉淡化过渡。
