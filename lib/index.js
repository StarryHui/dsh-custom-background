...
function buildCss(s, prevUrl, dur = 600) {
	...
	const layer = (image, z, opacity, extra) => "content:\"\";position:fixed;inset:-" + blur + "px;z-index:" + z + ";pointer-events:none;background-image:" + image + ";background-repeat:no-repeat;background-size:" + size + ";background-position:" + pos + ";transform:scale(" + zx + "," + zy + ");filter:blur(" + blur + "px);opacity:" + opacity + ";" + extra;
	let css = "html{background:#0d1017!important}body{background:transparent!important}";
	if (prevUrl) {
		const token = String(Date.now()) + Math.floor(Math.random() * 1e3);
		const prev = "url('" + prevUrl + "')";
		css += "body::before{" + layer(img, -1, "0", "animation:dshbg-in-" + token + " " + dur + "ms ease forwards;") + "}body::after{" + layer(prev, -2, String(op), "animation:dshbg-out-" + token + " " + dur + "ms ease forwards;") + "}@keyframes dshbg-in-" + token + "{from{opacity:0}to{opacity:" + op + "}}@keyframes dshbg-out-" + token + "{from{opacity:" + op + "}to{opacity:0}}";
	} else css += "body::before{" + layer(img, -1, String(op), "") + "}";
	css += "body{" + tokenBlock(DARK, a, oa) + "}body:not([data-ds-dark-theme]){" + tokenBlock(LIGHT, a, oa) + "}";
	return css;
}
...
function apply(ctx) {
	...
	if (fs === void 0 || settings === void 0) {
		console.error("[dsh-background] missing services", {
			fs: fs !== void 0,
	...
	const galleryItems = async (state) => {
		...
		try {
			...
			for (const entry of entries) {
				...
				items.push({
					name: entry.name,
					url: "/dsh-bg/img?p=" + encodeURIComponent(fs.processPath(entry.target))
				});
			}
			return {
				dir: state.galleryDir,
				items
			};
		} catch {
			return {
			...
			async handler(req, res) {
				try {
					...
					for (const pair of query.split("&")) {
						...
						if ((eq === -1 ? pair : pair.slice(0, eq)) === "p") p = decodeURIComponent(eq === -1 ? "" : pair.slice(eq + 1));
					}
					if (!p) {
					...
					const type = MIME[(p.split(".").pop() ?? "").toLowerCase()] ?? "application/octet-stream";
					const bytes = await fs.readBytes(target, void 0, MAX_IMG);
					res.writeHead(200, {
						"Content-Type": type,
		...
		ctx.effect(() => webServer.register({
			...
			async handler(req, res) {
				try {
					...
					const patch = body !== null && typeof body === "object" && !Array.isArray(body) && "patch" in body ? body.patch : body;
					if (patch !== null && typeof patch === "object" && !Array.isArray(patch)) await settings.update("background", patch);
					const payload = await compose(readState());
					res.writeHead(200, {
						"Content-Type": "application/json; charset=utf-8",
						"Cache-Control": "no-store"
		...
		ctx.effect(() => webServer.register({
			...
			async handler(req, res) {
				try {
					...
					const prev = params.get("prev") || void 0;
					const durRaw = Number(params.get("dur"));
					const dur = Number.isFinite(durRaw) ? Math.max(100, Math.min(3e3, durRaw)) : 600;
...
//#endregion
export { BackgroundSchema, apply, inject, name };
