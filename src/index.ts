...
interface FsTarget { targetKey: string; displayPath: string }
interface FsFace {
  resolve(path: string): Promise<FsTarget>
  processPath(target: FsTarget): string
  listDir(target: FsTarget): Promise<Array<{ name: string; target: FsTarget }>>
  readBytes(target: FsTarget, signal: unknown, maxBytes: number): Promise<Uint8Array>
  contains(parent: FsTarget, child: FsTarget): boolean
  writeText(target: FsTarget, content: string, expected?: unknown, signal?: unknown, sandboxPolicy?: unknown): Promise<unknown>
}
...
/** Minimal request face: URL, method, and a readable body (a real node:http IncomingMessage). */
...
async function readJsonBody(req: WebRequest, maxBytes = 64 * 1024): Promise<unknown> {
...
function backgroundUrl(s: BackgroundSettings): string | null {
  if (!s.current || !s.galleryDir) return null
...
function imageValue(s: BackgroundSettings): string {
 ...
 */
function buildCss(s: BackgroundSettings, prevUrl?: string, dur = 600): string {
  ...
  const layer = (image: string, z: number, opacity: string, extra: string) =>
    'content:"";position:fixed;inset:-' + blur + 'px;z-index:' + z + ';pointer-events:none;' +
...
function injectIntoHead(html: string, css: string): string {
  const style = '<style id="dsh-bg-durable">' + css + '</style>'
  const m = /<\/head>/i.exec(html)
  if (m) return html.slice(0, m.index) + style + html.slice(m.index)
  return html + style
}
...
export function apply(ctx: Context): void {
  ...
  const readState = (): BackgroundSettings => {
    const v = settings.get('background') as BackgroundSettings | undefined
  ...
  const galleryItems = async (state: BackgroundSettings) => {
    if (!state.galleryDir) return { dir: '', items: [] as Array<{ name: string; url: string }> }
    try {
      ...
      return { dir: state.galleryDir, items: [] as Array<{ name: string; url: string }> }
    }
  }

  const compose = async (state: BackgroundSettings) => {
      ...
      async handler(req, res) {
        try {
          ...
          for (const pair of query.split('&')) {
            ...
            if (key === 'p') p = decodeURIComponent(eq === -1 ? '' : pair.slice(eq + 1))
          }
          if (!p) { res.writeHead(400, { 'Content-Type': 'text/plain' }); res.end('missing p'); return }
          const state = readState()
          if (!state.galleryDir) { res.writeHead(404, { 'Content-Type': 'text/plain' }); res.end('no gallery'); return }
          const root = await fs.resolve(state.galleryDir)
          const target = await fs.resolve(p)
          if (!fs.contains(root, target)) { res.writeHead(403, { 'Content-Type': 'text/plain' }); res.end('forbidden'); return }
      ...
      async handler(req, res) {
        try {
          ...
          if (patch !== null && typeof patch === 'object' && !Array.isArray(patch)) {
            await settings.update('background', patch as object)
          }
          const payload = await compose(readState())
          res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store' })
          res.end(JSON.stringify(payload))
